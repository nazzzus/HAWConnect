...
router.get('/get/:userId', verifyToken, async (req, res) => {
  try {
    const sem4 = await Sem4Model.findOne({ userOwner: req.params.userId });
    res.status(200).json(sem4.modul);
  } catch (err) {
    console.log(err);
...
router.put('/toggle-belegt/:moduleId', async (req, res) => {
  try {
    const sem4 = await Sem4Model.findOneAndUpdate(
