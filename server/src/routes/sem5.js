...
router.get('/get/:userId', verifyToken, async (req, res) => {
  try {
    const sem5 = await Sem5Model.findOne({ userOwner: req.params.userId });
    res.status(200).json(sem5.modul);
  } catch (err) {
    console.log(err);
...
router.put('/toggle-belegt/:moduleId', async (req, res) => {
  try {
    const sem5 = await Sem5Model.findOneAndUpdate(
