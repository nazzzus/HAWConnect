...
function TableRow4({ module, image }) {
  ...
  const handleBelegtToggle = async () => {
    if (!isBestanden && !isPvlBestanden) {
      ...
      try {
        await axios.put(`http://localhost:3001/sem4/toggle-belegt/${module._id}`, {
          belegt: !isBelegt,
        });
      } catch (error) {
        console.log(error);
  ...
  const handlePVLBestandenToggle = async () => {
    ...
    try {
      await axios.put(`http://localhost:3001/sem4/toggle-pvl/${module._id}`, {
  ...
  const handleBestandenToggle = async () => {
    ...
    try {
      await axios.put(`http://localhost:3001/sem4/toggle-bestanden/${module._id}`, {
  ...
  const handleNoteSave = async () => {
    ...
    setIsLoading(true);
    try {
      await axios.put(`http://localhost:3001/sem4/update-note/${module._id}`, {
        note,
      });
    } catch (error) {
      console.log(error);
